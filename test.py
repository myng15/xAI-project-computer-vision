...
parser.add_argument('-c', '--num_classes', type=int,
    help='num_classes = 10 -> the model to be tested is trained on CIFAR-10, if num_classes = 100 -> CIFAR-100')
...
final_model_cp = torch.load(f'outputs/{output_folder}/final_model_{model_name}_{cp_datetime}.pth')
final_model_epoch = final_model_cp['epoch']
print(f"Last model was saved at {final_model_epoch} epochs\n")

# load the best model checkpoint
best_model_cp = torch.load(f'outputs/{output_folder}/best_model_{model_name}_{cp_datetime}.pth')
